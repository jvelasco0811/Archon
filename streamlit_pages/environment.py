...
def environment_tab():
    ...
    st.write(
        "Profiles allow you to store different sets of environment variables for different providers or use cases."
    )
    ...
    selected_llm_provider = st.selectbox(
        ...
        index=(
            llm_providers.index(st.session_state.llm_provider)
            if st.session_state.llm_provider in llm_providers
            else 0
        ),
    ...
    selected_embedding_provider = st.selectbox(
        ...
        index=(
            embedding_providers.index(st.session_state.embedding_provider)
            if st.session_state.embedding_provider in embedding_providers
            else 0
        ),
    ...
    with st.form("env_vars_form"):
        ...
        if selected_llm_provider == "Ollama" and (
            not current_api_key
            or profile_env_vars.get("LLM_PROVIDER", "") != selected_llm_provider
        ):
        ...
        placeholder = (
            current_api_key
            if current_api_key == "NOT_REQUIRED"
            else "Set but hidden" if current_api_key else ""
        )
        ...
        elif selected_llm_provider == "Ollama" and (
            not current_api_key or current_api_key == "NOT_REQUIRED"
        ):
        ...
        # EMBEDDING_BASE_URL
        embedding_base_url_help = (
            "Base URL for your embedding provider:\n\n"
            + "OpenAI: https://api.openai.com/v1\n\n"
            + "Ollama: http://localhost:11434/v1"
        )
        ...
        current_embedding_base_url = profile_env_vars.get(
            "EMBEDDING_BASE_URL",
            embedding_default_urls.get(selected_embedding_provider, ""),
        )

        # If provider changed or EMBEDDING_BASE_URL is empty, use the default
        if (
            not current_embedding_base_url
            or profile_env_vars.get("EMBEDDING_PROVIDER", "")
        ...
        # EMBEDDING_API_KEY
        embedding_api_key_help = (
            "API key for your embedding provider:\n\n"
            + "For OpenAI: https://help.openai.com/en/articles/4936850-where-do-i-find-my-openai-api-key\n\n"
            + "For Ollama, no need to set this unless you specifically configured an API key"
        )
        ...
        if selected_embedding_provider == "Ollama" and (
            not current_embedding_api_key
            or profile_env_vars.get("EMBEDDING_PROVIDER", "")
            != selected_embedding_provider
        ):
        ...
        elif selected_embedding_provider == "Ollama" and (
            not current_embedding_api_key or current_embedding_api_key == "NOT_REQUIRED"
        ):
        ...
        # If there's already a value, show asterisks in the placeholder
        placeholder = (
            "Set but hidden" if profile_env_vars.get("SUPABASE_SERVICE_KEY", "") else ""
        )
