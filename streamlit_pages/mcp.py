...
def generate_mcp_config(ide_type, python_path, server_script_path):
    ...
    if ide_type == "Windsurf":
        ...
        return (
            ...
            f"docker run -i --rm -e GRAPH_SERVICE_URL=http://host.docker.internal:8100 archon-mcp:latest",
        )
...
def mcp_tab():
    ...
    with col3:
        cline_button = st.button(
            "Cline/Roo Code", use_container_width=True, key="cline_button"
        )
    with col4:
        claude_button = st.button(
            "Claude Code", use_container_width=True, key="claude_button"
        )
    ...
    if st.session_state.selected_ide:
        ...
        python_config, docker_config = generate_mcp_config(
            selected_ide, python_path, server_script_path
        )
